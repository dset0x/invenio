...
def record_get_field_values(rec, tag, ind1="", ind2="", code="",
                            filter_subfield_code=None, filter_subfield_value="",
                            filter_subfield_mode="e"):
    """Return the list of values for the specified field of the record.

    A field consists of (tag, ind1, ind2, code).

    The list can be filtered. Use filter_subfield_code and filter_subfield_value
    to search only in fields that have these values inside them as a subfield.
    ...
    @param rec: a record dictionary
    @param tag: a 3 characters long string
    @param ind1: a 0 or 1 character long string
    @param ind2: a 0 or 1 character long string
    @param code: a 0 or 1 character long string
    @return: a list of strings
    """
    assert filter_subfield_mode in ('e', 's', 'r')
    assert len(tag) == 3
    assert all(len(x) <= 1 for x in (ind1, ind2, code))
    ...
    if code == "":
        ...
        for tag in tags:
            for field in rec[tag]:
                if (ind1 in ('%', field[1]) and ind2 in ('%', field[2]) and
                    field[3]):
                    output.append(field[3])
    else:
        # Code is specified. Consider all corresponding subfields

        def append_subfields():
            for subfield in field[0]:
                if code in ('%', subfield[0]):
                    output.append(subfield[1])
                ...
                if ind1 in ('%', field[1]) and ind2 in ('%', field[2]):
                    if filter_subfield_code:
                        if filter_subfield_mode == "e":
                            ...
                            string = dict(field[0]).get(filter_subfield_code, '')
                            if string.find(filter_subfield_value) > -1:
                                append_subfields()
                        elif filter_subfield_mode == "r":
                            string = dict(field[0]).get(filter_subfield_code, '')
                            if re.match(filter_subfield_value, string):
                                append_subfields()
                    else:
                        append_subfields()
